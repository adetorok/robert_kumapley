...
export function MultiStepForm({ anchorId = "contact-form", defaultIntent }: MultiStepProps) {
  const steps = formsContent.multistep.steps;
  const [current, setCurrent] = useState(0);
  const [status, setStatus] = useState<"idle" | "submitting" | "success" | "error">("idle");
  const [data, setData] = useState<Record<string, string | boolean>>(
    defaultIntent ? { intent: defaultIntent } : {},
  );
              ...
              <div key={field.name} className="space-y-2">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {field.options?.map((opt) => {
                    const selected = data[field.name] === opt;
                    return (
                      <button
                        type="button"
                        key={opt}
                        ...
                        className={`flex w-full items-center justify-between rounded-xl border px-4 py-3 text-left text-sm font-semibold transition ${
                          selected
                            ...
                            : "border-white/10 bg-white/5 text-white/80 hover:border-accent-400"
                        }`}
                        role="radio"
                        aria-checked={selected}
                      >
                        <span>{opt}</span>
                        <span className="text-xs text-white/60">{selected ? "Selected" : ""}</span>
                      </button>
                    );
                  })}
                </div>
